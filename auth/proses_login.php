...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if ($stmt->rowCount() == 1) {
            ...
            debug_log("User status check - Raw status: '" . ($user['status'] ?? 'not set') . "', Normalized: '" . $status . "'");
            
            if ($status !== 'active') {
                debug_log("Login failed - Account not active. User ID: " . $user['id']);
                throw new Exception("Akun Anda tidak aktif. Silakan hubungi administrator.");
            }
            
            debug_log("Account status check passed for user ID: " . $user['id']);
